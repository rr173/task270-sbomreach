...
func TestBug06_ReachableHopsAreIndependentCopies(t *testing.T) {
	...
	var reachable *model.ReachPath
	for _, p := range paths {
		if p.CVEID == "CVE-2024-0001" {
			reachable = p
		}
	}
	if reachable == nil || reachable.Status != model.PathReachable {
		t.Fatalf("missing reachable path")
	}
	if len(reachable.Hops) < 2 {
		t.Fatalf("hops corrupted or emptied: %#v", reachable.Hops)
	}
	sawHTTP := false
	for _, h := range reachable.Hops {
		if h.Target == "lib-http:HTTPRead" || h.Source == "lib-http:HTTPRead" {
			sawHTTP = true
		}
		if h.Target == "lib-ssl:WeakCipherInit" {
			t.Fatalf("reachable path reused blocked vuln hops: %#v", reachable.Hops)
		}
	}
	if !sawHTTP {
		t.Fatalf("reachable hops missing HTTPRead: %#v", reachable.Hops)
	}
