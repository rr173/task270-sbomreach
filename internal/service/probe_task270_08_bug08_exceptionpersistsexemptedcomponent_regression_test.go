...
func TestBug08_ExceptionPersistsExemptedComponent(t *testing.T) {

	rels, a, _, comps := openSvc(t)
	id := seedRel(t, rels, a)
	...
	if _, err := a.RegisterException(id, blocked.ID, "CVE-2024-0002", "legacy off", "eng"); err != nil {
		t.Fatal(err)
	}
	...
	for _, c := range list {
		if c.PURL == "pkg:golang/acme/lib-ssl@v3.0.0" && c.Status != model.ComponentExempted {
