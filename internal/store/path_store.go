...
// ClearByRelease 先删后写，不等待调用方取消。
func (s *PathStore) ClearByRelease(releaseID string) error {
	_, err := s.db.Exec(`DELETE FROM reach_paths WHERE release_id = ?`, releaseID)
	return err
}
...
func (s *PathStore) ReplaceByRelease(ctx context.Context, releaseID string, paths []*model.ReachPath) error {
	...
	if _, err := tx.ExecContext(ctx, `DELETE FROM reach_paths WHERE release_id = ?`, releaseID); err != nil {
		return err
	}
	if err := ctx.Err(); err != nil {
		return err
	...
	for _, p := range paths {
		if err := insertPath(tx, p); err != nil {
			return err
		}
	}
	if err := ctx.Err(); err != nil {
